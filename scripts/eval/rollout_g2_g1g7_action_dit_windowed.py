#!/usr/bin/env python3
"""Isolated G1-G7 Action-DiT LoRA G2 video/action window evaluation.

The persistent G1-G7 server must be started with
``--no-reset-cache-each-request``.
...
def main() -> None:
    ...
    # A window starts with the four-frame packet [start:start+4].  Its last
    # causal packet is [start+12:start+16], anchored at start+15; the decoded
    # GT/prediction segment is anchor:anchor+33, i.e. rows start+3..start+35.
    max_start = num_rows - (args.window_history * args.causal_blocks + args.window_future_frames - 1)
    ...
    for wi, window_start in enumerate(starts):
        ...
        for block_index in range(args.causal_blocks):
            ...
            result = np.asarray(
                client.infer(
                    {
                        "observation/top_head": videos["top_head"][indices],
                        "observation/hand_left": videos["hand_left"][indices],
                        "observation/hand_right": videos["hand_right"][indices],
        ...
        all_comparison_frames.extend(comparison_frames)
        ...
        logging.info(
            "window %d/%d start=%d video=%d frames action_mae=%.6f elapsed=%.1fs",
            wi + 1, len(starts), window_start, len(predicted_frames), window_reports[-1]["action_mae"], time.time() - started,
        )
    ...
    reports = _write_action_reports(args.output_dir, episode, starts, all_anchors, predicted, ground_truth)
    report = {
        "episode_index": episode,
