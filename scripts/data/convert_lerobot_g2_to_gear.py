...
materializes one parquet and one video per episode, forces all three camera
streams to the same CFR/resolution, writes GEAR metadata, and can physically
separate a held-out test set from the training set.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("tail", "random"),
        default="tail",
        help="tail reserves the final N source episodes; random uses --split-seed.",
    )
...
def load_source_episodes(
    ...
    task_by_index = {int(row["task_index"]): str(row["task"]).strip() for row in task_rows}
    ...
    for row in sorted(episode_rows, key=lambda item: int(item["episode_index"])):
        ...
        declared_tasks = [str(task).strip() for task in row.get("tasks", []) if str(task).strip()]
        task = declared_tasks[0] if declared_tasks else ""
        if not task and "task_index" in row:
            task = task_by_index.get(int(row["task_index"]), "")
        if not task:
            raise ValueError(f"Episode {source_index} has no language task")
        if len(set(declared_tasks)) > 1:
            raise ValueError(f"Episode {source_index} declares multiple tasks: {declared_tasks}")
        episodes.append(
            SourceEpisode(
...
def table_for_episode(data_dataset: pads.Dataset, episode: SourceEpisode) -> pa.Table:
    ...
    for optional in ("timestamp", "frame_index", "index", "task_index"):
