...
def audit(checkpoint: Path) -> list[str]:
    ...
    # Support both old LoRA-only checkpoints and new action-adapter-only
    # checkpoints. The latter intentionally freezes shared DiT/LoRA and only
    # stores state/action adapter parameters.
    ...
    has_lora = buckets["lora_A"] > 0 or buckets["lora_B"] > 0
    has_action_adapter = (
        buckets["action_encoder"] > 0
        or buckets["action_decoder"] > 0
        or buckets["state_encoder"] > 0
    )

    if has_lora:
        print("checkpoint_type: LoRA")
        if buckets["lora_A"] == 0:
    ...
    elif has_action_adapter:
        print("checkpoint_type: action_adapter")
    else:
        errors.append(
            "checkpoint contains neither LoRA weights nor action adapter weights"
        )
    for name, minimum in (
        ("action_encoder", 6),
        ("action_decoder", 4),
