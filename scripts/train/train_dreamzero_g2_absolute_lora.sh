...
"$PYTHON_BIN" -m torch.distributed.run \
    ...
    # x0 (decoded absolute-action reconstruction) is ~60-75x the flow loss in
    # scale under absolute targets. Weight it down so flow matching stays the
    # main action objective; x0 is only a small auxiliary.
    ++action_head_cfg.config.action_x0_loss_weight=0.01 \
