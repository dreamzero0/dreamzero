...
OUTPUT_DIR=${OUTPUT_DIR:-/data/wangk/checkpoints/dreamzero_g2_full_lora8_action_video_v1_4k}
...
relative = json.loads((root / "meta/relative_stats_dreamzero.json").read_text())
for key in ("left_joint_position", "right_joint_position"):
    if key not in relative:
        raise RuntimeError(f"Missing relative stats for {key}")
    if len(relative[key]["q01"]) != 7 or len(relative[key]["q99"]) != 7:
        raise RuntimeError(f"Motion-mask stats for {key} must be 7D")
...
echo "G2 mock shared-DiT LoRA training with full video+action objective"
...
"$PYTHON_BIN" -m torch.distributed.run \
    ...
    ++action_head_cfg.config.motion_mask_enabled=true \
    ++action_head_cfg.config.motion_mask_threshold_rad=0.03 \
    ++action_head_cfg.config.motion_mask_stats_path="$G2_DATA_ROOT/meta/relative_stats_dreamzero.json" \
    2>&1 | tee "$TRAIN_LOG"

echo "Completed G2 mock stationary-frame-masked LoRA training"
