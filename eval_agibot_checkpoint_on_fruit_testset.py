...
class Args:
    ...
    # Packaged AgiBot/G1 fruit held-out split. Override this path explicitly.
    test_data_root: str = "/data/training_data/teleop/agibot/fruit/test"

    # Output and sample selection.
    output_dir: str = "/data/wangk/dreamzero/agibot_fruit_testset_video_eval/checkpoint"
...
class AgiBotRoboarenaPolicy(DistributedRoboarenaPolicyBase):
    """Run an AgiBot checkpoint on an in-domain AgiBot fruit test split.

    This is intentionally video-only. The checkpoint's own mean AgiBot
    state fills required state fields, and native action output is discarded.
    """
    ...
    def __init__(
        ...
        logger.info(
            "[AGIBOT-FRUIT VIDEO TEST] video_frames=%s "
    ...
    def _normalize_video(
        ...
        # OpenCV decodes the fruit test MP4 files as BGR. DreamZero expects RGB.
    ...
    def _save_input_grid(
        ...
        grid_path = os.path.join(
            diagnostic_dir,
            f"agibot_fruit_input_request_{self._msg_index:06d}"
            f"_f{len(grids)}.mp4",
        )
        imageio.mimsave(
            grid_path,
    ...
    def _convert_observation(
        ...
        logger.info(
            "[AGIBOT-FRUIT VIDEO TEST] request=%s "
...
def _validate_test_dataset(root: Path) -> dict:
    _require_dir(root, "AgiBot fruit test split")
    info_path = root / "meta" / "info.json"
    embodiment_path = root / "meta" / "embodiment.json"
    _require_file(info_path, "fruit test info.json")
    _require_file(embodiment_path, "fruit test embodiment.json")
    ...
    if embodiment.get("embodiment_tag") != "agibot":
        raise ValueError(
            "Expected fruit test embodiment_tag='agibot', "
            f"got {embodiment}"
        )
    ...
    if missing:
        raise ValueError(
            "AgiBot fruit test split is missing camera features: "
            f"{sorted(missing)}; actual={sorted(actual_videos)}"
        )
...
def _fruit_display_grid_rgb(
    top_bgr: np.ndarray,
    left_bgr: np.ndarray,
    right_bgr: np.ndarray,
) -> np.ndarray:
    """Build a 2x2 diagnostic grid even when source cameras differ in size."""
    display_size = (320, 176)
    views = []
    for frame_bgr in (top_bgr, left_bgr, right_bgr):
...
def _run_one_test_sample(
    ...
    if args.prompt_override is not None:
        ...
        language_candidates = [
            ...
            "annotation.language.action_text",
            "annotation.human.action_text",
            "task",
        ]
        for key in language_candidates:
            if key in table.column_names:
                prompt = _unwrap_text(table[key][row_index].as_py())
                if prompt:
                    break
    ...
    _save_rgb_video(
        sample_dir / f"fruit_source_context_f{len(context_grids)}.mp4",
        context_grids,
        fps=30,
    )
    imageio.imwrite(
        sample_dir / "fruit_source_context_last.png",
        context_grids[-1],
    )
    ...
    _save_rgb_video(
        sample_dir / f"fruit_ground_truth_future_f{len(gt_grids)}.mp4",
        gt_grids,
        fps=30,
    )
    _save_rgb_video(
        sample_dir / f"fruit_ground_truth_future_f{len(gt_grids)}_slow.mp4",
        gt_grids,
        fps=5,
    )
    imageio.imwrite(
        sample_dir / "fruit_ground_truth_first.png",
        gt_grids[0],
    )
    imageio.imwrite(
        sample_dir / "fruit_ground_truth_last.png",
        gt_grids[-1],
    )

    # Resize each source camera independently to the exact raw resolution
    # expected by this AgiBot checkpoint.
    ...
    obs = {
        ...
        "prompt": prompt,
        "session_id": f"offline-fruit-{sample_name}-{uuid.uuid4()}",
    }

    wrapper._output_dir = str(sample_dir)
    wrapper._msg_index = 0
    logger.info(
        "[AGIBOT-FRUIT TESTSET] episode=%s row=%s context=%s "
    ...
    np.save(
        sample_dir / "ignored_agibot_actions.npy",
        ignored_actions,
    )
    ...
    comparison_native_path: str | None = None
    comparison_slow_path: str | None = None
    predicted_path: str | None = None
    predicted_slow_path: str | None = None
    ...
    if predicted_candidates:
        ...
        predicted_slow_file = (
            sample_dir
            / f"fruit_predicted_only_f{len(predicted_frames)}_slow.mp4"
        )
        _save_rgb_video(
            predicted_slow_file,
            predicted_frames,
        ...
        predicted_slow_path = str(predicted_slow_file)
        ...
        comparison_native = (
            sample_dir
            / f"agibot_predicted_vs_fruit_gt_f{compare_count}_30fps.mp4"
        )
        comparison_slow = (
            sample_dir
            / f"agibot_predicted_vs_fruit_gt_f{compare_count}_slow.mp4"
        )
        _save_rgb_video(
            comparison_native,
            comparison_frames,
        ...
        imageio.imwrite(
            sample_dir / "agibot_predicted_vs_fruit_gt_first.png",
            comparison_frames[0],
        )
        imageio.imwrite(
            sample_dir / "agibot_predicted_vs_fruit_gt_last.png",
            comparison_frames[-1],
        )
        comparison_native_path = str(comparison_native)
        comparison_slow_path = str(comparison_slow)

    summary = {
        "checkpoint": str(Path(args.model_path).resolve()),
        "checkpoint_embodiment": "agibot",
        "visual_source_embodiment": "agibot",
        "visual_source_task": "fruit",
        ...
        "prompt": prompt,
        "dataset_state_used_for_model_conditioning": False,
        "predicted_video": predicted_path,
        "predicted_video_slow": predicted_slow_path,
        "comparison_video_30fps": comparison_native_path,
        "comparison_video_slow": comparison_slow_path,
        "generated_frame_count": 9 if predicted_path else 0,
        "note": (
            "The model emits one 9-frame decoded video chunk in this evaluator. "
            "The slow files change playback speed only; they do not extend the "
...
def main(args: Args) -> None:
    if args.embodiment_tag.lower() != "agibot":
        raise ValueError(
            "This evaluator runs AgiBot checkpoints on an AgiBot fruit test split; "
    ...
    logger.info(
        ...
        "visual_source=agibot_fruit test=%s episodes=%s eval_offsets=%s "
